...
class EnhancedAutoTagger:
    ...
    def __init__(self, use_advanced_features: bool = True):
        ...
        self.model_version = "enhanced_v2.0" if use_advanced_features else "standard_v1.0"
    ...
    def train(self, transactions: pd.DataFrame, target_accuracy: float = 0.80) -> Dict[str, Any]:
        """Train the enhanced model with advanced features"""
        
        logger.info(f"Training {self.model_version} on {len(transactions)} transactions")
        ...
        if accuracy >= target_accuracy:
            logger.info(f"✅ Target accuracy achieved: {accuracy:.4f} >= {target_accuracy}")
        else:
            logger.warning(f"⚠️ Target accuracy not met: {accuracy:.4f} < {target_accuracy}")
        ...
        enhanced_metrics = {
            **metrics,
            ...
            "feature_engineering_level": "advanced" if self.use_advanced_features else "standard"
        }
    ...
    def predict(self, transactions: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        # Make predictions using underlying tagger
        try:
            if hasattr(self.tagger, 'predict') and callable(self.tagger.predict):
    ...
    def _fallback_predict(self, transactions: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for _, row in transactions.iterrows():
            ...
            if any(keyword in merchant for keyword in ["zomato", "swiggy", "food", "restaurant", "cafe"]):
                category = "Food & Dining"
                confidence = 0.7
            elif any(keyword in merchant for keyword in ["uber", "ola", "metro", "petrol", "fuel"]):
                category = "Transport"
                confidence = 0.7
            elif any(keyword in merchant for keyword in ["amazon", "flipkart", "shopping", "store"]):
                category = "Shopping"
                confidence = 0.7
            elif any(keyword in merchant for keyword in ["electricity", "gas", "water", "mobile", "internet"]):
                category = "Bills & Utilities"
                confidence = 0.7
            elif any(keyword in merchant for keyword in ["movie", "cinema", "netflix", "entertainment"]):
    ...
    def save_model(self, model_name: str = "enhanced_auto_tagger") -> str:
        ...
        try:
            ...
            if hasattr(self.tagger, 'save_model'):
                run_id = self.tagger.save_model(version=self.model_version)
            else:
                # Fallback MLflow save
                with mlflow.start_run() as run:
                    mlflow.log_param("model_version", self.model_version)
                    mlflow.log_param("advanced_features", self.use_advanced_features)
...
def create_test_data(n_samples: int = 1000) -> pd.DataFrame:
    ...
    merchants = {
        "Food & Dining": ["Zomato", "Swiggy", "McDonald's", "Subway", "Cafe Coffee Day"],
        "Transport": ["Uber", "Ola", "Metro Card", "IRCTC", "Petrol Pump"],
        "Shopping": ["Amazon", "Flipkart", "Big Bazaar", "Reliance Store", "DMart"],
        "Bills & Utilities": ["Electricity Board", "Jio", "Airtel", "Gas Agency", "Water Board"],
        "Entertainment": ["BookMyShow", "Netflix", "Spotify", "Gaming Store", "YouTube"],
        "Other": ["ATM Withdrawal", "Bank Transfer", "Unknown Merchant", "Cash Deposit"]
    }
...
if __name__ == "__main__":
    ...
    test_data = create_test_data(n_samples=2000)
    ...
    print(f"Features: {metrics.get('feature_count', 'unknown')}")
    ...
    if 'feature_groups' in insights:
        for group, count in insights['feature_groups'].items():
            print(f"  {group}: {count} features")    def __init__(self, use_advanced_features: bool = True):
        self.use_advanced_features = use_advanced_features
